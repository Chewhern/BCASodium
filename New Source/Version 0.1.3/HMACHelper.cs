...
namespace BCASodium
{
    public static class HMACHelper
    {
        ...
        public static Boolean VerifyHMAC(IDigest myDigest, Byte[] Message, Byte[] SHMAC ,Byte[] Key, Boolean ClearKey = false)
        {
            ...
            myHMAC.DoFinal(HMAC, 0);
            GCHandle CipherTextMACGCHandle = GCHandle.Alloc(HMAC, GCHandleType.Pinned);
            GCHandle MACGCHandle = GCHandle.Alloc(SHMAC, GCHandleType.Pinned);
            try
            {
                SodiumHelper.Sodium_Memory_Compare(CipherTextMACGCHandle.AddrOfPinnedObject(), MACGCHandle.AddrOfPinnedObject(), HMAC.Length);
            }
            catch
            {
