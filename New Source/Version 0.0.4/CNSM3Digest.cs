...
namespace BCASodium
{
    public static class CNSM3Digest
    {
        ...
        public static Byte[] ComputeHashForNonFinalizedData(Byte[] Message, SM3Digest MyDigest, Boolean IsFinal = false)
        {
            if (Message == null)
            {
                throw new ArgumentException("Error: Message must not be null");
        ...
        public static Byte[] ComputeMAC(Byte[] Message, Byte[] Key, Boolean ClearKey = false)
        {
            ...
            Byte[] HashedCMessageP2 = CNSM3Digest.ComputeHashForNonFinalizedData(ConcatedMessageP2, MyDigest);
            Byte[] ConcatedMessage = K1.Concat(HashedCMessageP2).ToArray();
            Byte[] MAC = CNSM3Digest.ComputeHashForNonFinalizedData(ConcatedMessage, MyDigest, true);
            MyDigest.Reset();
            if (ClearKey == true)
            {
