...
namespace BCASodium
{
    public static class CNSM4
    {
        ...
        public static Byte[] CBC_Mode_Decrypt(Byte[] CipherText, Byte[] Key, Boolean ClearKey = false)
        {
            ...
            while (Loop < DividedBlocks)
            {
                ...
                else
                {
                    Array.Copy(ActualCipherText, BlockSize * Loop, SubCipherText, 0, 16);
                    TempPlainText = Unsafe_Decrypt(SubCipherText, Key);
                    PreviousCipherText = new Byte[16];
                    Array.Copy(ActualCipherText, (BlockSize * (Loop - 1)), PreviousCipherText, 0, 16);
        ...
        private static Byte[] GCM_CTR_Mode_Encrypt(Byte[] Message, Byte[] Nonce, Byte[] Key, Boolean ClearKey = false)
        {
            if (Message == null)
            {
                throw new ArgumentException("Error: Message can't be null");
        ...
        private static Byte[] GCM_CTR_Mode_Decrypt(Byte[] CipherText, Byte[] Nonce, Byte[] Key, Boolean ClearKey = false)
        {
            return GCM_CTR_Mode_Encrypt(CipherText, Nonce, Key, ClearKey);
        }

        private static Byte[] GCM_GenerateGMAC(Byte[] CipherText, Byte[] Nonce, Byte[] Key, Byte[] AdditionalData = null, Boolean ClearKey = false)
        {
            if (CipherText == null)
            {
                throw new ArgumentException("Error: Cipher Text can't be null");
        ...
        public static Byte[] GCM_Decrypt(Byte[] CipherTextGMAC, Byte[] Nonce, Byte[] Key, Byte[] AdditionalData = null, Boolean ClearKey = false)
        {
            ...
            Byte[] GMAC = GCM_GenerateGMAC(CipherText, Nonce, Key, AdditionalData);
            Byte[] PlainText = new Byte[] { };
            GCHandle CTextGMACGCHandle = GCHandle.Alloc(CTextGMAC, GCHandleType.Pinned);
            GCHandle GMACGCHandle = GCHandle.Alloc(GMAC, GCHandleType.Pinned);
            try 
            {
                SodiumHelper.Sodium_Memory_Compare(CTextGMACGCHandle.AddrOfPinnedObject(), GMACGCHandle.AddrOfPinnedObject(), CTextGMAC.Length);
