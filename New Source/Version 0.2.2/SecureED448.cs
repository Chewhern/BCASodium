...
namespace BCASodium
{
    public static class SecureED448
    {
        ...
        //Will need to visit "KeyPair" variant in the future
        //because current version doesn't really utilize libsodium at all..
        ...
        public static Byte[] Sign(Byte[] SecretKey, Byte[] Message, Byte[] Context,Boolean ClearKey = false)
        {
            ...
            if(Context == null) 
            {
                throw new ArgumentException("Error: Context must not be null");
            }
            else 
            {
                if (Context.Length > 255) 
                {
                    throw new ArgumentException("Error: Context must not be bigger than 255 bytes");
                }
            }
            Byte[] Signature = new Byte[Ed448.SignatureSize];
            Byte[] PreHashedMessage = SHAKEDigest.ComputeHashForED448(Message);

            Ed448.SignPrehash(SecretKey, 0, Context, PreHashedMessage, 0, Signature, 0);
            if (ClearKey == true)
            {
        ...
        public static Boolean Verify(Byte[] PublicKey, Byte[] Message, Byte[] Signature, Byte[] Context)
        {
            ...
            CanVerify = Ed448.VerifyPrehash(Signature, 0, PublicKey, 0, Context, PreHashedMessage, 0);
