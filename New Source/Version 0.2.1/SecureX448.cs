...
namespace BCASodium
{
    public class SecureX448
    {
        public static X448RevampedKeyPair GenerateX448RevampedKeyPair()
        {
            ...
            if (SodiumHelper.Sodium_Is_Zero(X448PK) == 1)
            {
                throw new CryptographicException("Error: Kindly regenerate the keypair to prevent X448 public key was all 0s");
            }
        ...
        public static Byte[] CalculateSecret(Byte[] OthersPublicKey, Byte[] YourPrivateKey, Boolean ClearPrivateKey = false)
        {
            ...
            if (result != 0) 
            {
                throw new CryptographicException("Error: The computed shared secret is 0. Kindly check back or regenerate the sender or recipient's public key");
            }
