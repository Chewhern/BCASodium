...
namespace BCASodium
{
    ...
    {
        //If RNG was preferred, first enum and 5th enum is recommended.
        public enum PaddingSelection 
        {
            ISO10126d2,
            ISO7816d4,
            PKCS7,
        ...
        public static Byte[] Sign(Byte[] SecretKey, Byte[] Message, Byte[] Context,PaddingSelection selection = PaddingSelection.X923,Boolean ClearKey = false)
        {
            ...
            {
                //Actually, there might not need to declare a new variable
                //but i feel safer-ish in doing this instead of concatted
                //message length - filling data length
                OriginalMessageLength = Message.Length;
                FillingData = new Byte[115 - Message.Length];
                Message = Message.Concat(FillingData).ToArray();
                if (selection == PaddingSelection.ISO10126d2)
                {
                    PaddingClass.ISO10126D2AddPadding(Message, OriginalMessageLength);
                }
                else if (selection == PaddingSelection.ISO7816d4)
                {
                    PaddingClass.ISO7816D4AddPadding(Message, OriginalMessageLength);
                }
                else if (selection == PaddingSelection.PKCS7)
                {
                    PaddingClass.PKCS7AddPadding(Message, OriginalMessageLength);
                }
                else if (selection == PaddingSelection.TBC)
                {
                    PaddingClass.TBCAddPadding(Message, OriginalMessageLength);
                }
                else if (selection == PaddingSelection.X923)
                {
                    PaddingClass.X923AddPadding(Message, OriginalMessageLength);
                }
                else
                {
                    PaddingClass.ZeroByteAddPadding(Message, OriginalMessageLength);
                }
            }
        ...
        public static Boolean Verify(Byte[] PublicKey, Byte[] Message, Byte[] Signature, Byte[] Context, PaddingSelection selection = PaddingSelection.X923)
        {
