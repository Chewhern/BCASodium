...
namespace BCASodium
{
    public class SecureX448
    {
        ...
        public static KeyPair GenerateKeyPair()
        {
            ...
            MyGeneralGCHandle = GCHandle.Alloc(X448SK, GCHandleType.Pinned);
            SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), X448SK.Length);
            MyGeneralGCHandle.Free();

            MyGeneralGCHandle = GCHandle.Alloc(X448PK, GCHandleType.Pinned);
            SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), X448PK.Length);
            MyGeneralGCHandle.Free();
        ...
        public static Byte[] GeneratePublicKey(Byte[] X448SK, Boolean ClearKey = false)
        {
            ...
            if (ClearKey == true)
            {
                GCHandle MyGeneralGCHandle = GCHandle.Alloc(X448SK, GCHandleType.Pinned);
                SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), X448SK.Length);
                MyGeneralGCHandle.Free();
            }
            return X448PK;
        ...
        public static Byte[] CalculateSecret(Byte[] OthersPublicKey, Byte[] YourPrivateKey, Boolean ClearPrivateKey = false)
        {
            ...
            {
                GCHandle MyGeneralGCHandle = GCHandle.Alloc(YourPrivateKey, GCHandleType.Pinned);
                SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), YourPrivateKey.Length);
                MyGeneralGCHandle.Free();
            }
            return SharedSecret;
        }
